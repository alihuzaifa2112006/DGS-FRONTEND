...
export default function Signup() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    setLoading(true)
    await wait(1100)
    setSession({ email: String(data.get('email')), name: String(data.get('name')), at: Date.now() })
    router.push('/app')
  }
      ...
      <form onSubmit={onSubmit} className="space-y-4" noValidate>
        ...
        <label className="flex items-start gap-2 text-[13px] text-ink-600">
          ...
          <span>
            ...
            <Link href="/help" className="font-semibold text-ink-900 underline decoration-ink-300 underline-offset-2">Terms</Link>.
          </span>
        </label>
